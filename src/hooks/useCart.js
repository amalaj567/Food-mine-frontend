...
export default function CartProvider({ children }) {
  ...
  const initWishlist = getWishlistFromLocalStorage();
  const [wishlistItems, setWishlistItems] = useState(initWishlist.items);
  ...
  const addToWishlist = food => {
    const wishlistItem = wishlistItems.find(item => item.food.id === food.id);
    if (wishlistItem) {
      return;
    }
    setWishlistItems([...wishlistItems, { food }]);
  };
  ...
  const removeFromWishlist = foodId => {
    setWishlistItems(wishlistItems.filter(item => item.food.id !== foodId));
